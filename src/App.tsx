...
function Sidebar({ data, route, enabled, navigate }: { data: LoadedData; route: Route; enabled: Record<ModuleKey, boolean>; navigate: (route: Route) => void }) {
  ...
  return <aside className="sidebar">
    <button className="brand" onClick={() => navigate("dashboard")}><span className="brand-mark">G</span><span>{data.config.siteName}</span></button>
    <nav aria-label="主导航">{nav.map(({ key, label, icon: Icon }) => <button key={key} className={route === key ? "active" : ""} onClick={() => navigate(key)}><Icon size={19} /><span>{label}</span></button>)}</nav>
    <div className="sidebar-spacer" />
    <button className={route === "settings" ? "active" : ""} onClick={() => navigate("settings")}><Settings size={19} /><span>设置</span></button>
